...
// le vecteur contiendra condition|séquence|condition|...|séquence
    ...
    vector<Noeud*> noeuds;
    vector<Noeud*> noeudsSinon;
    
    testerEtAvancer("si");
    testerEtAvancer("(");
    Noeud* conditionSi = expression();
    noeuds.push_back(conditionSi);
    testerEtAvancer(")");
    Noeud* sequenceSi = seqInst();
    noeuds.push_back(sequenceSi);
    
    while(m_lecteur.getSymbole()=="sinonsi"){
        testerEtAvancer("sinonsi");
        testerEtAvancer("(");
        Noeud* conditionSinonsi = expression(); 
        noeuds.push_back(conditionSinonsi);
        testerEtAvancer(")");
        Noeud* sequenceSinonSi = seqInst();
        noeuds.push_back(sequenceSinonSi);
    }
    // pas forcément un sinon.
    if (m_lecteur.getSymbole()=="sinon"){
        testerEtAvancer("sinon");
        Noeud* sequenceSinon = seqInst(); 
        noeudsSinon.push_back(sequenceSinon);
    }
    
    
    testerEtAvancer("finsi");
    return new NoeudInstSiRiche(noeuds, noeudsSinon);
}
...
Noeud* Interpreteur::instPour() {
    ...
    testerEtAvancer("(");
    Noeud* affectationDeb = affectation();
    testerEtAvancer(";");
